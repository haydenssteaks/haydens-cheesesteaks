...
export default function OrderClient({
  ...
  const handlePaymentSuccess = useCallback(
    async (token: string) => {
      ...
      setPaymentError("");
      try {
        // Send items (not amount) to the payment API — server calculates the price
        ...
        if (data.success) {
          // Create the order record via server action (also calculates price server-side)
    ...
    [customerName, customerEmail, customerPhone, notes, eventId, quantities]
  );
          ...
          <div className="mx-auto max-w-md px-4 text-center">
            <div className="bg-white rounded-2xl p-8 shadow-sm mb-6">
              ...
              <p className="text-sm text-charcoal/50 mt-3">
                {itemCount} item{itemCount !== 1 ? "s" : ""} for pickup at{" "}
                {venueName}
              </p>
            </div>
            <p className="text-charcoal/40 text-sm mb-6">Show this page at the event to pick up your order.</p>
            <Link
              href="/events"
      ...
      <section className="bg-teal py-12 md:py-16 text-center">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-cream/50 text-sm">
            {eventName} &mdash; Pickup at {venueName}
          </p>
        </div>
      </section>
